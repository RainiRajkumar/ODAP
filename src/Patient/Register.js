...
function Register(){
    ...
    //     const response = await axios.get('http://localhost:8086/exists/'+email);
    ...
    // }
    // else if(response){
    //     console.error("error occured while checking email: ", response);
    // }
        ...
        if(patientName === '' ){
          ...
          else if(!inputmobileNumber(phoneNumber)){
    ...
    try {
      const response = await axios.post('http://localhost:8092/patient/register' ,data);
        setPatientName('');
        setPhoneNumber('');
        setEmail('');
